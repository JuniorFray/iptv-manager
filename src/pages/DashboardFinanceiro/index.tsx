...
export default function DashboardFinanceiro() {
  ...
  const vencidos   = clientes.filter(c => { const d = parseDate(c.vencimento); return d && d < hoje })
  const fatVencido = vencidos.reduce((acc, c) => acc + parseFloat(c.valor || '0'), 0)
  const proxVencer = ativos.filter(c => { const d = parseDate(c.vencimento); return d && d >= hoje && d <= limite })
  ...
  return (
    <div>
      <div style={{ marginBottom: '28px' }}>
        <h1 className="page-title" style={{ color: 'white', fontSize: '28px', fontWeight: 'bold', margin: 0 }}>Dashboard Financeiro</h1>
        <p style={{ color: 'rgba(255,255,255,0.5)', marginTop: '4px', fontSize: '14px' }}>Visão geral de receita e vencimentos</p>
      </div>
      ...
      <div style={{ display: 'flex', gap: '16px', flexWrap: 'wrap', marginBottom: '24px' }}>
        ...
        <div style={cardStyle('99,102,241')}>
          <p style={labelStyle}>Faturamento Esperado</p>
          ...
          <p style={subStyle}>{proxVencer.length} clientes vencem nos próximos {periodo} dias</p>
        </div>
      ...
      <div style={{ display: 'flex', gap: '16px', flexWrap: 'wrap' }}>
        ...
        <div className="glass-card" style={{ flex: 2, minWidth: '280px', overflow: 'hidden' }}>
          <div style={{ padding: '20px 20px 0' }}>
            <h3 style={{ color: 'white', margin: '0 0 16px', fontSize: '16px', fontWeight: '600' }}>Receita por Servidor</h3>
          </div>
        ...
        <div className="glass-card" style={{ flex: 3, minWidth: '320px', overflow: 'hidden' }}>
          <div style={{ padding: '20px 20px 0' }}>
            <h3 style={{ color: 'white', margin: '0 0 16px', fontSize: '16px', fontWeight: '600' }}>
              Vencendo nos próximos {periodo} dias
          ...
          <div className="table-responsive">
            <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: '480px' }}>
              ...
              <tbody>
                {proxVencer.length === 0 ? (
                  <tr><td colSpan={5} style={{ padding: '30px', textAlign: 'center', color: 'rgba(255,255,255,0.3)' }}>Nenhum cliente.</td></tr>
                ) : proxVencer.map(c => (
                  <tr key={c.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                    ...
                    <td style={{ padding: '12px 20px', color: '#fbbf24', fontWeight: '600', fontSize: '13px' }}>{c.vencimento}</td>
                    <td style={{ padding: '12px 20px', color: '#4ade80', fontWeight: '600', fontSize: '13px' }}>
